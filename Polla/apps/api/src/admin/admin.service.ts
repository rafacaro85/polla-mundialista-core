...
export class AdminService {
  ...
  async runMigration(key: string) {
    ...
    if (key !== SECRET) {
      throw new Error('Unauthorized: Invalid migration key');
    }
...
END $$;
    ...
    try {
      ...
      return {
        success: true,
        message: '✅ Database schema updated successfully via emergency endpoint',
      };
    } catch (error) {
      return {
        success: false,
        message: `❌ Emergency migration failed: ${error.message}`,
      };
    }
  }
}
