...
async function checkThirds() {
    ...
    try {
        ...
        const groups = await repo.query(`SELECT DISTINCT "group" FROM matches WHERE phase='GROUP'`);

        console.log(`Grupos encontrados: ${groups.length}`);

        // Obtener tabla de terceros manual
        const allMatches = await repo.find({ where: { phase: 'GROUP', status: 'FINISHED' } });

        // Calcular stats rapidito
        const teams: any = {};
        allMatches.forEach(m => {
            // ... lógica de puntos ...
            if (!teams[m.homeTeam]) teams[m.homeTeam] = { pts: 0, gf: 0, gd: 0, group: m.group };
            if (!teams[m.awayTeam]) teams[m.awayTeam] = { pts: 0, gf: 0, gd: 0, group: m.group };
        ...
        thirds.forEach((t, i) => console.log(`${i + 1}. ${t.team} (${t.pts} pts) - Grupo ${t.group}`));
