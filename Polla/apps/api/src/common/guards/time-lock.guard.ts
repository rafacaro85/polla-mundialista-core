...
export class TimeLockGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (match.isManuallyLocked) {
            throw new ForbiddenException('🔒 BLOQUEADO: Este partido ha sido bloqueado manualmente por el administrador. No se aceptan predicciones.');
        }
        ...
        if (now >= lockTime) {
            const minutesUntilMatch = Math.floor((matchDate.getTime() - now.getTime()) / 60000);
            ...
            if (minutesUntilMatch <= 0) {
                ...
                throw new ForbiddenException(`⏰ TIEMPO AGOTADO: Las apuestas cierran 10 minutos antes del inicio. Faltan ${minutesUntilMatch} minutos para el partido.`);
            }
        }
