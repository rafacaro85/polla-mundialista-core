...
export class ExpandSchemaForAdvancedFeatures1764649438287 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."leagues_status_enum_old" AS ENUM('ACTIVE', 'LOCKED', 'FINISHED')`,
    );
    await queryRunner.query(
      `ALTER TABLE "leagues" ALTER COLUMN "status" DROP DEFAULT`,
    );
    await queryRunner.query(
      `ALTER TABLE "leagues" ALTER COLUMN "status" TYPE "public"."leagues_status_enum_old" USING "status"::"text"::"public"."leagues_status_enum_old"`,
    );
    await queryRunner.query(
      `ALTER TABLE "leagues" ALTER COLUMN "status" SET DEFAULT 'ACTIVE'`,
    );
    await queryRunner.query(`DROP TYPE "public"."leagues_status_enum"`);
    await queryRunner.query(
      `ALTER TYPE "public"."leagues_status_enum_old" RENAME TO "leagues_status_enum"`,
    );
    await queryRunner.query(
      `ALTER TABLE "leagues" DROP COLUMN "prize_image_url"`,
    );
    await queryRunner.query(
      `ALTER TABLE "leagues" DROP COLUMN "prize_details"`,
    );
    await queryRunner.query(
      `ALTER TABLE "leagues" DROP COLUMN "branding_logo_url"`,
    );
    await queryRunner.query(
      `ALTER TABLE "league_participants" DROP COLUMN "trivia_points"`,
    );
