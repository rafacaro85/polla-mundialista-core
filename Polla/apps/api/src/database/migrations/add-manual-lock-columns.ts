...
async function runMigration() {
  try {
    ...
    if (hasIsLocked.length > 0) {
      console.log(
        '🔄 Renaming column isLocked to isManuallyLocked in matches table...',
      );
      ...
      console.log(
        '⚠️ isLocked column not found, checking if isManuallyLocked exists...',
      );
      ...
      if (hasIsManuallyLocked.length === 0) {
        ...
        console.log(
          '✅ isManuallyLocked column already exists in matches table',
        );
      }
    }
    ...
    if (hasPhaseColumn.length === 0) {
      console.log(
        '➕ Adding is_manually_locked column to knockout_phase_status table...',
      );
      await queryRunner.query(`
      ...
      console.log(
        '✅ is_manually_locked column already exists in knockout_phase_status table',
      );
    }
