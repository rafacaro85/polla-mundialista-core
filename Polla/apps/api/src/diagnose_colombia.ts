...
async function diagnose() {
    ...
    try {
        ...
        matches.forEach((m: any) => {
            console.log(`${m.date.toISOString().split('T')[0]} | ${m.homeTeam} (${m.homeScore}) vs (${m.awayScore}) ${m.awayTeam} | ${m.status}`);
        });
        ...
        matches.forEach((m: any) => {
            if (m.status !== 'FINISHED') {
                console.log(`⚠️ Partido ${m.homeTeam} vs ${m.awayTeam} NO está FINISHED: ${m.status}`);
                return;
            }
