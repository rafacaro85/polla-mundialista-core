...
async function fixCalendar() {
  ...
  try {
    ...
    if (july4.length === 0) {
      ...
      const july4Range = await dataSource.query(
        `SELECT id, phase, "homeTeamPlaceholder" FROM matches WHERE date >= $1 AND date <= $2`,
        [start, end],
      );
    ...
    console.log(
      `Encontrados ${r32Matches.length} partidos de ROUND_32 para reagendar.`,
    );
    ...
    console.log(
      '✅ Calendario ROUND_32 actualizado a fechas FIFA (28 Jun - 03 Jul).',
    );
  } catch (e) {
    console.error(e);
  } finally {
