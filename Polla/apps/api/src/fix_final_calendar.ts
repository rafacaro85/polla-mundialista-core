...
async function fixFinalPhases() {
  ...
  try {
    ...
    const semis = await dataSource.query(
      `SELECT id FROM matches WHERE phase = 'SEMI' ORDER BY date ASC`,
    );

    if (semis.length >= 2) {
      await dataSource.query(
        `UPDATE matches SET date = '2026-07-14T20:00:00Z' WHERE id = $1`,
        [semis[0].id],
      );
      await dataSource.query(
        `UPDATE matches SET date = '2026-07-15T20:00:00Z' WHERE id = $1`,
        [semis[1].id],
      );
    ...
    if (finals.length > 0) {
      await dataSource.query(
        `UPDATE matches SET date = '2026-07-19T20:00:00Z' WHERE id = $1`,
        [finals[0].id],
      );
    ...
    const thirdPlace = await dataSource.query(
      `SELECT * FROM matches WHERE phase = '3RD_PLACE'`,
    );
    if (thirdPlace.length === 0) {
      ...
      await dataSource.query(
        `UPDATE matches SET date = '2026-07-18T20:00:00Z' WHERE phase = '3RD_PLACE'`,
      );
