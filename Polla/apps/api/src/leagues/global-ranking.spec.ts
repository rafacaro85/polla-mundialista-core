...
describe('LeaguesService - Global Ranking Isolation', () => {
    ...
    // Mock Data
    const mockUserWC = { id: 'user-wc', nickname: 'WC Fan', email: 'wc@test.com' } as User;
    const mockUserUCL = { id: 'user-ucl', nickname: 'UCL Fan', email: 'ucl@test.com' } as User;
    ...
    it('getGlobalRanking should RETURN only active participants for the requested tournament', async () => {
        ...
        jest.spyOn(userRepo.manager, 'query').mockImplementation(async (sql: string, params: any[]) => {
            ...
            if (sql.includes('CASE WHEN "isJoker"')) {
                ...
                // Only return points for UCL user
                 return [{ userId: mockUserUCL.id, regular_points: 10, joker_points: 0 }];
            }
