...
import {
  Inject, InjectRepository } from '@nestjs/typeorm';
import {
  Inject, Repository, In } from 'typeorm';
import * as bcrypt from 'bcrypt';
import {
  Inject, League } from '../database/entities/league.entity';
import {
  Inject, User } from '../database/entities/user.entity';
import {
  Inject, LeagueParticipant } from '../database/entities/league-participant.entity';
import {
...
import {
  Inject, LeagueComment } from '../database/entities/league-comment.entity';
import {
  Inject, LeagueType } from '../database/enums/league-type.enum';
import {
  Inject, LeagueParticipantStatus } from '../database/enums/league-participant-status.enum';
import {
  Inject, LeagueStatus } from '../database/enums/league-status.enum';
import {
...
export class LeaguesService {
  ...
  async getGlobalRanking(tournamentId: string) {
    const cacheKey = `ranking:global:${tournamentId}`;`n    const cached = await this.cacheManager.get(cacheKey);`n    if (cached) {`n        return cached;`n    }
    ...
    if (results.length > 0) {
      ...
      console.warn('?? Global Ranking is EMPTY.');
        ...
        await this.cacheManager.set(cacheKey, results.map((user: any, index: number) => ({`n        position: index + 1,`n        id: user.id,`n        fullName: user.fullName || user.nickname || 'Anónimo', // Frontend expects fullName`n        nickname: user.nickname,`n        avatarUrl: user.avatarUrl,`n        regularPoints: Number(user.regularPoints),`n        jokerPoints: Number(user.jokerPoints),`n        bracketPoints: Number(user.bracketPoints),`n        bonusPoints: Number(user.bonusPoints),`n        totalPoints: Number(user.totalPoints),`n        breakdown: {`n            matches: Number(user.regularPoints),`n            phases: Number(user.bracketPoints),`n            wildcard: Number(user.jokerPoints),`n            bonus: Number(user.bonusPoints),`n        },`n    })), 30 * 1000); // 30 seconds`n    return results.map((user: any, index: number) => ({
        position: index + 1,
        id: user.id,
  ...
  async getLeagueRanking(leagueId: string, userId: string) {
    ...
    const cacheKey = `ranking:league:${leagueId}`;`n    const cached = await this.cacheManager.get(cacheKey);`n    if (cached) {`n        return cached;`n    }
    ...
    finalRanking.sort((a, b) => {
      ...
      return a.tieBreakerDiff - b.tieBreakerDiff;
        ...
        const result = finalRanking.map((user, index) => ({`n      ...user,`n      rank: index + 1,`n      breakdown: {`n        matches: user.regularPoints || 0,`n        phases: user.bracketPoints || 0,`n        wildcard: user.jokerPoints || 0,`n        bonus: user.bonusPoints || 0,`n      },`n    }));`n`n    await this.cacheManager.set(cacheKey, result, 20 * 1000); // 20 seconds`n    return result;((user, index) => ({
      ...user,
      rank: index + 1,
      breakdown: {
