...
async function lockFuturePhases() {
    ...
    try {
        ...
        console.log('✅ Fases futuras bloqueadas (ROUND_16, QUARTER, SEMI, FINAL).');

        const status = await dataSource.query(`SELECT phase, is_unlocked FROM knockout_phase_status`);
