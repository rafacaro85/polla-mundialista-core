...
export class MailService {
  ...
  constructor() {
    ...
    console.log(
      `📡 [MailService] SMTP Config: ${process.env.SMTP_HOST || 'smtp.gmail.com'}:${process.env.SMTP_PORT || 587} (User: ${user ? '✅ set' : '❌ missing'})`,
    );
  ...
  async sendVerificationEmail(to: string, code: string) {
    ...
    if (process.env.RESEND_API_KEY) {
      const result = await this.sendViaResend(
        to,
        'Verifica tu correo electrónico - Polla Mundialista',
        html,
      );
      if (result?.success) {
        console.log(
          '✅ [MailService] Verification email sent via Resend to %s (ID: %s)',
          to,
          result.messageId,
        );
        return result;
    ...
    if (!smtpUser) {
      console.warn(
        '⚠️ [MailService] SMTP_USER is not set and Resend not configured. Email cannot be sent.',
      );
      return {
        success: false,
  ...
  async sendResetPasswordEmail(to: string, resetLink: string) {
    ...
    if (process.env.RESEND_API_KEY) {
      const result = await this.sendViaResend(
        to,
        'Recupera tu contraseña - Polla Mundialista',
        html,
      );
      if (result?.success) {
        console.log(
          '✅ [MailService] Reset password email sent via Resend to %s (ID: %s)',
          to,
          result.messageId,
        );
      }
    ...
    try {
      ...
      console.error(
        '❌ [MailService] Error enviando correo de recuperación:',
        to,
        error,
      );
      return { success: false, error: error.message, code: error.code };
