...
async function run() {
  ...
  try {
    ...
    const hasHeading = res.rows.some(r => r.column_name === 'brand_color_heading');
    console.log('¿Tiene brand_color_heading?:', hasHeading);

    if (!hasHeading) {
        console.log('ERROR CRÍTICO: Mi diagnóstico anterior decía que sí, pero ahora dice que no.');
        console.log('Intentando añadir de nuevo con SQL puro...');
        await client.query('ALTER TABLE leagues ADD COLUMN IF NOT EXISTS brand_color_heading VARCHAR(255);');
        await client.query('ALTER TABLE leagues ADD COLUMN IF NOT EXISTS brand_color_bars VARCHAR(255);');
