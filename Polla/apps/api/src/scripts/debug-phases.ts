...
async function bootstrap() {
  ...
  all.filter(p => p.tournamentId === 'WC2026').forEach(p => {
      console.log(`[WC2026] ${p.phase} (Unlocked: ${p.isUnlocked})`);
  });

  console.log('--- UCL2526 Phases ---');
  all.filter(p => p.tournamentId === 'UCL2526').forEach(p => {
      console.log(`[UCL2526] ${p.phase} (Unlocked: ${p.isUnlocked})`);
  });

  // Check for 'PLAYOFF' in WC2026
  const invalidWC = all.filter(p => p.tournamentId === 'WC2026' && p.phase === 'PLAYOFF');
  ...
  const invalidUCL = all.filter(p => p.tournamentId === 'UCL2526' && (p.phase === 'GROUP' || p.phase === 'ROUND_32'));
