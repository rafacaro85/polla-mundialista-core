...
async function diagnoseReset() {
  ...
  try {
    ...
    const lpCols = await ds.query(
      "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'league_participants'",
    );
    console.log('league_participants columns:');
    console.table(lpCols);

    const pCols = await ds.query(
      "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'predictions'",
    );
    console.log('predictions columns:');
    console.table(pCols);

    const ubCols = await ds.query(
      "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'user_brackets'",
    );
    console.log('user_brackets columns:');
    console.table(ubCols);

    const kpsCols = await ds.query(
      "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'knockout_phase_status'",
    );
