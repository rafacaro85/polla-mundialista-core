...
async function run() {
    ...
    try {
         ...
         ucl.forEach(m => console.log(`ID: ${m.id} | Home: "${m.homeTeam}" | Flag: "${m.homeFlag}"`));
    } catch(e) {
        console.error("UCL Query Failed:", e.message);
        
        // Fallback: search by ID or date?
        // Let's dump everything with 'Madrid' in it
        const madrid = await repo.createQueryBuilder("match")
            .where("match.homeTeam ILIKE :name", { name: '%Madrid%' })
            .getMany();
        console.log("Matches with Madrid:", madrid.map(m => m.homeTeam));
    }
