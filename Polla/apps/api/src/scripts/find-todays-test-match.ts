...
async function findTodaysTestMatch() {
  try {
    console.log('🔍 Searching for matches playing TODAY (Monday, Feb 16, 2026)...\n');
    ...
    const goodMatches = allFixtures.filter((fixture: any) => {
      ...
      const matchDateColombia = new Date(matchDate.toLocaleString('en-US', { timeZone: 'America/Bogota' }));
      const hoursUntil = (matchDateColombia.getTime() - nowColombia.getTime()) / (1000 * 60 * 60);
      
      // Matches starting in 2-8 hours
      return hoursUntil >= 2 && hoursUntil <= 8;
    });

    console.log(`✅ Found ${goodMatches.length} match(es) starting in 2-8 hours:\n`);

    if (goodMatches.length === 0) {
      console.log('⚠️  No matches in ideal time window. Showing all today\'s matches:\n');
