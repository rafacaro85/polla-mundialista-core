...
async function main() {
  ...
  if (!process.env.DATABASE_URL) {
      console.error('DATABASE_URL not found in env. Checking hardcoded fallback or exit.');
  ...
  for (const m of matches) {
      ...
      if (m.phase !== 'GROUP') {
           // Check Home
          if (m.homeTeam && (m.homeTeam.startsWith('PLA_') || m.homeTeam.includes('_'))) {
              console.log(`Cleaning Home Team ${m.homeTeam} in Match ${m.id} (${m.phase})`);
              m.homeTeam = ''; 
              m.homeFlag = '';
              changed = true;
          }
          ...
          // Check Away
          if (m.awayTeam && (m.awayTeam.startsWith('PLA_') || m.awayTeam.includes('_'))) {
              console.log(`Cleaning Away Team ${m.awayTeam} in Match ${m.id} (${m.phase})`);
              m.awayTeam = '';
              m.awayFlag = '';
              changed = true;
          }
