...
const UCL_LOGOS_MAPPING: Record<string, string> = {
  ...
  "Girona FC": "https://upload.wikimedia.org/wikipedia/en/9/90/For_Girona_FC.svg",
...
async function fixUclLogos() {
    try {
        ...
        const dbTeamNames = teamsRaw.map((r: any) => r.name).filter((n: string) => n && n.length > 2);
        
        console.log(`ℹ️ Equipos encontrados en DB (Partidos UCL): ${dbTeamNames.length}`);
