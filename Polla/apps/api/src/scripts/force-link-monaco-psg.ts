...
async function forceLink() {
    console.log('📡 Fetching specifically for Monaco vs Paris Saint Germain on 2026-02-17...');
    ...
    try {
        ...
        const match = fixtures.find((f: any) => 
            (f.teams.home.name.toLowerCase().includes('monaco') || f.teams.away.name.toLowerCase().includes('monaco')) &&
            (f.teams.home.name.toLowerCase().includes('paris') || f.teams.away.name.toLowerCase().includes('paris'))
        );
        ...
        console.log(`🎯 Match Found in API: ${match.teams.home.name} vs ${match.teams.away.name} -> ID: ${apiId}`);
        ...
        console.log(`📝 Found DB Match: ${dbMatch.homeTeam} vs ${dbMatch.awayTeam} (ID: ${dbMatch.id})`);
