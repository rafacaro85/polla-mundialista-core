...
const LOGO_MAP: Record<string, string> = {
  ...
  'Shakhtar Donetsk':
    'https://upload.wikimedia.org/wikipedia/en/a/a1/FC_Shakhtar_Donetsk.svg', // Keep Wiki/Generic for now
...
async function run() {
  ...
  // VERIFY WC
  const wc = await queryRunner.query(
    `SELECT "homeTeam", "homeFlag" FROM matches WHERE "homeTeam" ILIKE '%Argentina%' LIMIT 1`,
  );
  console.log('Verify Argentina:', JSON.stringify(wc[0], null, 2));

  // VERIFY UCL
  const ucl = await queryRunner.query(
    `SELECT "homeTeam", "homeFlag" FROM matches WHERE "homeTeam" ILIKE '%Real Madrid%' LIMIT 1`,
  );
