  ...
  try {
    ...
    for (const target of targets) {
      const match = fixtures.find(
        (f: any) =>
          (f.teams.home.name
            .toLowerCase()
            .includes(target.home.toLowerCase()) ||
            target.home
              .toLowerCase()
              .includes(f.teams.home.name.toLowerCase())) &&
          (f.teams.away.name
            .toLowerCase()
            .includes(target.away.toLowerCase()) ||
            target.away
              .toLowerCase()
              .includes(f.teams.away.name.toLowerCase())),
      );

      if (match) {
        console.log(
          `🎯 Match Found: ${match.teams.home.name} vs ${match.teams.away.name} -> ID: ${match.fixture.id}`,
        );
        updates.push({
          id: match.fixture.id,
          home: target.home,
        ...
        console.warn(
          `⚠️  Match NOT Found in API: ${target.home} vs ${target.away}`,
        );
      }
    }
    ...
    if (updates.length > 0) {
      ...
      for (const upd of updates) {
        ...
        console.log(
          `📝 Updated DB for ${upd.home} vs ${upd.away}: ${res.rowCount} row(s)`,
        );
      }
      await client.end();
    }
