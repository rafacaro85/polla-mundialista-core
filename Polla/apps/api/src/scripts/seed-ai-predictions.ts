...
async function seedAiPredictions() {
  try {
    ...
    for (let i = 0; i < matches.length; i++) {
      ...
      try {
        console.log(
          `${progress} Generating prediction for: ${match.homeTeam} vs ${match.awayTeam}`,
        );
        ...
        console.log(
          `✅ ${progress} Saved: ${prediction.predictedScore} (${prediction.confidence})`,
        );
        successCount++;
        ...
        if (
          ...
          console.log(
            '⚠️ Rate limit detected. Waiting 120 seconds for quota reset...\n',
          );
          await sleep(120000);
