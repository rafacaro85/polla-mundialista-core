...
async function verify() {
  ...
  try {
    ...
    const wcRes = await client.query(
      `SELECT COUNT(*) FROM matches WHERE "tournamentId" = 'WC2026'`,
    );
    const wcCount = parseInt(wcRes.rows[0].count, 10);

    // Count UCL2526 Matches
    const uclRes = await client.query(
      `SELECT COUNT(*) FROM matches WHERE "tournamentId" = 'UCL2526'`,
    );
    const uclCount = parseInt(uclRes.rows[0].count, 10);

    // Check for anomalies
    const anomalyRes = await client.query(
      `SELECT COUNT(*) FROM matches WHERE "tournamentId" NOT IN ('WC2026', 'UCL2526')`,
    );
