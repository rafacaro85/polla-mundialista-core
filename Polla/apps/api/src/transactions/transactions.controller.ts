import { Controller, Get, Post, Patch, Param, Body, Res, NotFoundException, UseGuards, Request, UseInterceptors, UploadedFile, BadRequestException, Query } from '@nestjs/common';
...
export class TransactionsController {
    ...
    @UseGuards(JwtAuthGuard)
    @Get('my-latest')
    async getMyLatestTransaction(@Request() req: any, @Query('scope') scope?: string, @Query('leagueId') leagueId?: string) {
        if (scope === 'account') {
            return this.transactionsService.findLatestAccountTransaction(req.user.id);
        }
        if (leagueId) {
            return this.transactionsService.findLatestLeagueTransaction(req.user.id, leagueId);
        }
    ...
    async uploadTransaction(
        ...
        @Body() body: { amount?: number, referenceCode?: string, leagueId?: string }
    ) {
        if (!file) {
    ...
    async updateStatus(
        ...
        return this.transactionsService.updateStatus(id, body.status, body.adminNotes);
    }
    ...
    async downloadVoucher(@Param('id') id: string, @Res() res: any) {
        ...
        if (!transaction.user && !transaction.league) {
            ...
            if (!transaction.user) {
                throw new NotFoundException('Datos de usuario de la transacción faltantes');
            }
        }
