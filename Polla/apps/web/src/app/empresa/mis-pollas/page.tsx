    ...
    return (
        <div className={`group relative bg-[#1E293B] border rounded-2xl p-5 flex flex-col gap-4 transition-all duration-300 shadow-lg hover:shadow-[0_0_20px_rgba(0,230,118,0.08)] ${
            ...
            <div className="flex gap-2 mt-auto">
                {/* GESTIONAR STUDIO — solo para admins */}
                {league.isAdmin && (
                    <Link
                        href={league.isEnterpriseActive ? `/leagues/${league.id}/studio` : '#'}
                        onClick={(e) => !league.isEnterpriseActive && e.preventDefault()}
                        className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all duration-200 ${
                            league.isEnterpriseActive 
                            ? "bg-[#00E676]/10 border border-[#00E676]/20 text-[#00E676] hover:bg-[#00E676] hover:text-[#0F172A]" 
                            : "bg-white/5 border border-white/5 text-slate-600 cursor-not-allowed grayscale opacity-50"
                        }`}
                    >
                        <Settings2 size={13} /> {league.isEnterpriseActive ? 'Studio' : (league.status === 'REJECTED' ? 'REINTENTAR' : 'Bloqueado')}
                    </Link>
                )}
                ...
                {league.status === 'REJECTED' && (
                    <button
                        ...
                        className="h-10 w-10 shrink-0 flex items-center justify-center rounded-xl bg-red-500/10 border border-red-500/20 text-red-500 hover:bg-red-500 hover:text-white transition-all shadow-lg"
