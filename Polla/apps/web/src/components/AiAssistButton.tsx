...
export function AiAssistButton({ matches, onPredictionsGenerated }: AiAssistButtonProps) {
    ...
    return (
        <button
            ...
            disabled={loading}
            className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-purple-600 to-[var(--brand-primary,#4f46e5)] hover:brightness-110 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed group border border-white/10"
        >
            {loading ? (
