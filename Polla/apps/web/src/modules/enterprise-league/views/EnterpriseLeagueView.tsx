...
export const EnterpriseLeagueView = ({ leagueId }: { leagueId: string }) => {
    ...
    if (data.league.userStatus === 'PENDING_PAYMENT' || data.league.userStatus === 'REJECTED') {
      // mostrar módulo de pago
      const currentLeague = data.league;
      return (
        <div className="absolute inset-0 z-50 bg-[#0F172A] flex items-start justify-center pt-24 p-6">
          <PaymentMethods 
            leagueId={currentLeague.id} 
            amount={currentLeague.price} 
    ...
    if (data.league.userStatus === 'PENDING' && data.league.userStatus !== 'PENDING_PAYMENT') {
        const currentLeague = data.league;
        return (
